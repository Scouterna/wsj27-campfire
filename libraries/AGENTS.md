...
- `ui` – the design system: the tokens, the five themes, the display face, Storybook's own scaffolding, and the components, of which there is one today.
- `utils` – the small pure helpers more than one package needs. There is one today.
...
- A library depends on the platform, on React where it draws, and on nothing in the workspace – today none of the three has a workspace dependency at all.
...
└── src/
    ├── assets.d.ts                a .css import is Vite's business, told to TypeScript
    ├── components/logo/           Logo.tsx, Logo.css, Logo.stories.tsx
    ├── foundations/theme/         Theme.ts – the five names, the type, and isTheme
    └── storybook/                 the two decorators, stories.css, Introduction.mdx
```
...
- **Raw values live in `assets/styles/tokens.css` and nowhere else.** A component names a role – `--color-theme-bright`, `--typography-display-font_family` – and never a hex code. Color and the display face are tokens today; spacing, radius, and the type scale are not, so a component spells its own numbers out until a second component wants the same value.
- **The five themes** are the contingent's color identities: blue, brown, green, red, and yellow, with blue the default. A theme is described in two places – its value in `tokens.css`, where `[data-theme="<name>"]` repoints `--color-theme-bright` rather than every rule that reads it, and its name in `src/foundations/theme/Theme.ts` – so adding one is a change in both. No color crosses into a shell today: each shell carries the tonal blue for its launch surface, and that is the whole of it. The design is that a theme crosses as a name and each shell maps it to a palette of its own, which puts the five identities in three places and makes a change to one a change in all three.
- **A theme name arrives from outside** – a stored preference, a query parameter, Storybook's toolbar – so `isTheme` checks it rather than asserting it. Nothing outside Storybook sets `data-theme` today, which is why every browser shows blue.
...
One export, `stringOrFallback`, and the bar for a second is that a second package already wants it. A helper with one caller belongs beside that caller, where it can change without touching a package everything depends on.
...
- **The `fetch` wrapper and the HTTP error the data layer is designed around are not here yet** ([Data layer](../docs/guidebook/architecture/layers/data.md)). When they land, the wrapper distinguishes three outcomes – the request reached nothing, the service answered a refusal, the answer was not JSON – names the URL in the error and keeps `cause`, and the refusal carries its status, because a caller telling a 403 from a 404 is the difference between a second request and a wrong screen.
- Nothing here imports React or touches the DOM, and nothing here knows a feature. A helper that needs either is not a utility.
...
- Components are proved in Storybook rather than by rendering tests, which is why the `ui` Vitest project runs in Node and not a DOM: the only logic it tests is pure – the five theme names and the check that reads one from outside.
