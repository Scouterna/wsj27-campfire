...
└── src/
    ...
    ├── behavior/                  transitions.ts – the view-transition direction contract
    ├── components/                one directory per component – button/, card/, logo/, navigationbar/, overflowmenu/, pageactions/, pageheading/, pageoutline/, pagetitle/, profilepill/, row/, sidemenu/, tabbar/
...
- **Raw values live in `assets/styles/tokens.css` and nowhere else.** A component names a role – `--color-theme-bright`, `--typography-display-font_family` – and never a hex code. Color, spacing, radius, the display face, and the type scale are tokens; a screen's own tuned poster values stay in the screen until a second surface wants them.
...
- **Font sizes come from the `--font-size-*` tokens**, each `calc(<N>rem / 17)` against the 17-point base `base.css` sets – never a bare `px` – so Dynamic Type carries the reader's own text size through every role, and the rem math lives in `tokens.css` alone. Weights are tokens too: `--font-weight-strong` for emphasized chrome text and a button's label, and the display face's own beside the face.
