...
- **Only dev is deployed, and by hand.** `campfire.wsj27.scouterna.net` runs both services and the web image built from this repository, on Kubernetes in Azure ([ADR 027](/decisions/027-run-the-back-end-on-kubernetes-in-azure)), but the image is handed over rather than pulled – the release pipeline stops at publishing it. `campfire.wsj27.se` answers with a placeholder page, not Campfire.
