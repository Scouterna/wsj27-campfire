...
- **The commit type moves the version, and the files it touches say which one.** The web image, the Android shell, and the Apple shell each carry a CalVer version, `YEAR.FEATURE.PATCH`, worked out from the commits touching that artifact since its last release tag: a `feat` or a `!` moves the feature segment, a `fix` or a `revert` the patch, and the other types move nothing. Nothing in the tree carries the number, and nobody edits one – the scheme is in [Maintenance](../maintenance/release) ([ADR 034](/decisions/034-version-each-artifact-from-its-own-commits)).
