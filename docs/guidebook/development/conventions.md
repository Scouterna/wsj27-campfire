...
- **The commit type moves the version.** Campfire is on CalVer, `YEAR.FEATURE.PATCH`, at `2026.2.1`: a `feat` bumps the feature segment, a `fix` the patch segment, and the other types move nothing. The scheme and what to do with it are in [Maintenance](../maintenance/).
