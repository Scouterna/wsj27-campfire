...
| `release_web.yml`        | Push to `main` touching the web application; also by hand                             | Works out the version, builds and publishes the image, tags it, and moves `:dev`, as a deployment in the `dev` environment |
...
The commits are the trigger. `release_web.yml` checks out the full history, runs the version rule in `scripts/release/` over the commits since the last `web-v*` tag that touch its own trigger paths, and gets a version or nothing ([ADR 034](/decisions/034-version-each-artifact-from-its-own-commits)). It always publishes `:main` and `:sha-<short>` – for rollback, and for pointing a deployment at one exact commit. With a version, the image is built with it as a build argument and also published as `:<version>`; the git tag `web-v<version>` is created last, so it exists only once the image it names is published; and `:dev` is moved to that image after the tag, in a job of its own that runs only when a version was earned. A merge with a `feat` or a `fix` on the web's paths is therefore a release, and nothing has to be remembered. The skills are the exception: they are keyed on `metadata.version`, because a skill's version tracks the freshness of its content rather than the commits ([ADR 025](/decisions/025-publish-agent-skills-as-versioned-releases)).

`promote_web.yml` is the one workflow with an input. It takes a version, checks the tag and the image exist, and copies the image's manifest to `:prod` without building anything, so the digest that reaches prod is the digest dev ran. It needs `packages: write` and nothing about the cluster.

Both workflows record what they move as GitHub deployments. The job that moves `:dev` runs in the `dev` environment and the promotion in the `prod` environment, and running in an environment is what makes every run a deployment record, linking to the site it serves – `campfire.wsj27.scouterna.net` and `campfire.wsj27.se` ([ADR 035](/decisions/035-promote-the-web-by-moving-environment-tags)). A merge that earns no version skips the dev job, so it records no deployment.
