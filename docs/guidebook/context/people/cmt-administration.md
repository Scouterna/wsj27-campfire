...
Administration signs into the same application every leader gets and sees a wider slice of it. Their section menu offers the whole list of participants rather than one unit: a search across name, belonging, and member number, and filter chips for the participation roles. Opening a person shows what the back-end sent for them, each section rendering only when its data arrived.
...
The front-end does not act on which function it is: any `wsj27:cmt` role reads as management, so every function gets the same screens and the same wording. What each function may read is the service's call – every `wsj27:cmt` role reads the whole list of participants at the basic level, and the health answers need a [health grant](./cmt-health) the administration does not carry.
