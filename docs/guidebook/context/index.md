...
The scouts and the IST are in the register but not on the diagram as users. Nothing rules out a participant-facing surface later, and if one arrives it is a person and a line added to the model rather than a footnote.
...
| Participants service | Serves the contingent's register behind `/api/project`         | [`participants-service.md`](/context/systems/participants-service) |
| ScoutID              | Scouterna's identity provider, where every sign-in happens     | [`scoutid.md`](/context/systems/scoutid)                           |
| Scoutnet             | Scouterna's member registry, the source of the register        | [`scoutnet.md`](/context/systems/scoutnet)                         |
