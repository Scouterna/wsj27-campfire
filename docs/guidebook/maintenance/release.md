...
The version in the root `package.json` is `2026.5.0`: Calendar Versioning in the shape `YEAR.FEATURE.PATCH` ([ADR 007](/decisions/007-version-with-calver-on-a-rebased-history)).
...
Two other files carry the same number by hand – `CAMPFIRE_VERSION_NUMBER` in `apps/apple/config/Shared.xcconfig` and `versionName` in `apps/android/config/app/build.gradle.kts`, both `2026.5.0`, both on build number 1. Nothing keeps the three in step. A version bump is three edits, or an inconsistency waiting to be found.
