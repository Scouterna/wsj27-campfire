...
The first feature: a leader opens the participants section and sees the people in their unit. It is the flow every later module that fetches anything is built to look like, so it is worth reading twice. Locally, the [mock](../../testing/mock) serves the register and the gates below.

## What the viewer is allowed to ask for

The participants service lists the register one troop at a time and gates every answer by the caller's roles ([Applications](../applications)). There is no endpoint that returns everyone a person may see, so the client has to know who is reading before it can ask anything.
...
sequenceDiagram
  ...
  Query-->>Hook: The register, with its scope
...
It also composes. A leader asks for their own troop and is done. The contingent management walks the register the only way the service offers it: the leaders' listing names every troop, then each troop, the IST, and the management itself are fetched and merged by member number. An empty listing answers as if nothing is there, which for a composed register means no rows rather than a failure.
...
The register that comes back carries its own scope – everyone, one unit, or nobody – so the screen can say what it is showing rather than infer it from a row count.
...
| The request never reached anyone | The cached register is shown if there is one, and a failure if not   |
| The service refuses the listing  | The screen says the register could not be read, not that it is empty |
