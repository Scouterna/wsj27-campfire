...
- **A theme is a name, not a stylesheet.** The contingent has five color identities – blue, brown, green, red, and yellow, with blue the default – and a theme is one primitive, `--color-theme-bright`, that every rule naming the unit's color reads. `[data-theme="<name>"]` swaps the pointer rather than every rule that reads it, and the attribute is stamped before React boots, so a returning brown-unit leader does not flash blue on the way in. A color never crosses the bridge to a shell; the name does, and each shell holds its own palette.
