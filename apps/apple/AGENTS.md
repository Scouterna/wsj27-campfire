...
- The version is CalVer, `2026.1.0`, the same string the root `package.json` and the Android `versionName` carry, so one release reads the same everywhere. Bump it in `Shared.xcconfig`, which is why that file exists.
