...
`AppChrome` is the router's `InnerWrap` and the session gate, in `routes.tsx` – one `Gate` state: `asking`, `signed-in`, `signed-out`, or `ending`. It asks `currentUser(queryClient)` once at boot, renders nothing until the answer – no spinner, and no flash of the sign-in screen past a signed-in person – and then mounts the sign-in screen in the remembered theme or the signed-in application: the theme resolved unit-first, the roles made ambient through `RolesProvider`, and one branch on `host.tier` – after `adoptCacheOwner` has handed the cache to the signed-in member number. While signed in it keeps listening through the authentication module's `subscribeToSession`: a session the service ends puts the gate at `ending` – the application unmounted and nothing on screen – while `forgetCache` empties the client and the store, then at `signed-out`, so the sign-in screen stands at the address the person was at and sign-in returns there; a session that changes hands reloads the page, and the boot gate adopts the new owner ([ADR 033](../../docs/decisions/033-recover-an-ended-session-at-the-query-client-and-the-gate.md)). `SignedInChrome` starts the service's keep-alive script and the module's `watchExpiry` together, and stops the watcher when it unmounts:
...
The client and its defaults are real: `src/query.ts` holds the one `QueryClient`, and **every service read goes through it** – the gate hands it to `currentUser`, and the screens' hooks read it from the provider the gate mounts. The utils `fetch` is the transport inside a query function, never a path around the cache. The per-query persister, the `withSession` wrapper it runs every network read under – a 401 asks the authentication module who is signed in, and the same person still signed in runs the read once more, so no screen ever renders a refusal ([ADR 033](../../docs/decisions/033-recover-an-ended-session-at-the-query-client-and-the-gate.md)) – and `adoptCacheOwner` live beside the client, buster and all. A restore from the store never asks, because it never reached the network; a module declares nothing for any of this. [Data layer](../../docs/guidebook/architecture/layers/data.md) is the whole design, down to the DTO boundary the converters validate at.
