...
Today there is no router and no table: `main.tsx` mounts one module's screen at no address at all.
...
None of it is installed. TanStack Query is decided and absent, no module has a `data/` directory, and `libraries/utils` exports `stringOrFallback` where the `fetch` wrapper will go. [Data layer](../../docs/guidebook/architecture/layers/data.md) is the whole design, down to the DTO boundary the modules validate at.
...
- A theme is a name, not a stylesheet – `blue`, `brown`, `green`, `red`, or `yellow`, to be resolved once above the screens and written to `data-theme`. `libraries/ui` already carries the five palettes and the `data-theme` rules that swap them; nothing resolves one yet. Never send a color over the bridge; the shells hold their own palette per name.
