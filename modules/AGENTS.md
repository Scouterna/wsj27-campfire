...
`src/index.ts` is a deliberate, narrow list, opening with a JSDoc block saying what the module hands out. Domain types, queries, and screens stay internal unless the application genuinely needs them – anything reaching for a domain type is reaching past the boundary rather than through it. Today home exports its one screen, journey exports its screen and its widget table, authentication exports its sign-in screen beside the session client the gate asks, the three doorways the session opens after boot – `withSession` for the query client to run every read under, `subscribeToSession` for the gate to hear the session end or change owner, and `watchExpiry` for the gate to start beside the service's keep-alive ([ADR 033](../docs/decisions/033-recover-an-ended-session-at-the-query-client-and-the-gate.md)) – the route table that holds the profile page, and the `SignOutProvider` the gate hands that page its way out through – the `User` the answer decodes to is `utils`' type, so every module can read it – and participants exports its route table and section label – its screens mount through the table rather than by name – plus its widget table, and the viewer machinery the gate seeds.
